 ...
 *
 * @version 0.0.1 by TyomaVader; modified to allow rendering notifications similar to how wowmapper did.
 * @date 28.07.2023
 */
...
public:
	...
	inline void setIconTexture(const ImTextureID icon)
	{
		this->iconId = icon;
	}
...
public:
	...
	inline ImTextureID getIconTexture()
	{
		return this->iconId;
	}
...
namespace ImGui
{
	...
	inline void RenderNotifications(ImTextureID imgBg, ImTextureID imgBottom, ImTextureID imgHeader, ImTextureID imgIcon)
	{
		...
		{
			ImGuiToast* toast = &notifications[i];
			...
			const ImVec4 textColor = ImVec4(1, 1, 1, opacity);

			const char* content = toast->getContent();

			// Estimate wrapped text height
			float wrapWidth = 340.0f;
			ImVec2 textSize = ImGui::CalcTextSize(content, nullptr, false, wrapWidth);
			int lineCount = ceil(textSize.x / wrapWidth);
			float contentHeight = textSize.y * lineCount;

			float topMargin = 70.0f;
			float extraPadding = 10.0f;
			float backgroundHeight = contentHeight + topMargin + extraPadding;

			// Unique window name
			char windowName[64];
			sprintf_s(windowName, "##ToastWindow%d", i);


			const ImVec2 mainWindowPos = ImGui::GetMainViewport()->Pos;
			// Position bottom-right
			ImVec2 toastPos = ImVec2(
				mainWindowPos.x + mainWindowSize.x + 20.0f,
				mainWindowPos.y + mainWindowSize.y - 20.0f - heightOffset
			);

			ImGui::SetNextWindowPos(toastPos, ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			ImGui::SetNextWindowSize(ImVec2(400, backgroundHeight));
			...
			// Background
			drawList->AddImage(imgBg, winPos, ImVec2(winPos.x + 400, winPos.y + backgroundHeight),
				ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, int(opacity * 255)));

			// Bottom overlay
			drawList->AddImage(imgBottom,
				ImVec2(winPos.x, winPos.y + backgroundHeight),
				ImVec2(winPos.x + 400, winPos.y + backgroundHeight + 100),
				ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, int(opacity * 255)));

			// Header
			drawList->AddImage(imgHeader, winPos, ImVec2(winPos.x + 400, winPos.y + 83),
				ImVec2(0, 0), ImVec2(1, 1), ImColor(255, 255, 255, int(opacity * 255)));

			// New dimensions
			float iconSize = 42.0f;
			float iconX = winPos.x + 50;
			float iconY = winPos.y + 12; // still shifted up

			drawList->AddImageRounded(toast->getIconTexture(),
				ImVec2(iconX, iconY),
				ImVec2(iconX + iconSize, iconY + iconSize),
				ImVec2(0, 0), ImVec2(1, 1),
				ImColor(255, 255, 255, int(opacity * 255)),
				iconSize / 2.0f); // keep it circular


			// Header text
			ImGui::SetCursorPos(ImVec2(105,	25));
			ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1, 1, 1, opacity));
			ImGui::Text("%s", toast->getTitle());
			ImGui::PopStyleColor();
			...
			heightOffset += backgroundHeight + 10.0f; // Stack spacing
		}
	}
}
